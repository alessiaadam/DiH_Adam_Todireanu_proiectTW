...
$sql = "SELECT p.* FROM plants p WHERE p.id = ?";
...
try {
    ...
    $stmt_media = $pdo->prepare("SELECT file_path, type FROM media WHERE plant_id = ?");
...
<body>
    ...
    <main>
        <article id="plantDetails">
            ...
            <?php if ($plant['user_id'] == $_SESSION['user_id'] || ($_SESSION['user_role'] ?? '') === 'admin'): ?>
                <a href="edit_plant.php?id=<?php echo $plant['id']; ?>"
                    style="display: inline-block; margin-left: 10px; padding: 8px 16px; background-color: #4CAF50; color: white; border-radius: 4px; text-decoration: none;">
                    Editează Planta
                </a>
            <?php endif; ?>
            ...
            <div class="plant-detail-image-container">
                <?php if (!empty($all_media)): ?>
                    <?php foreach ($all_media as $media_item): ?>
                        <?php if ($media_item['type'] === 'video'): ?>
                            <video controls style="width: 100%; margin-bottom: 10px;">
                                <source src="<?php echo htmlspecialchars($media_item['file_path']); ?>">
                            </video>
                        <?php else: ?>
                            <img src="<?php echo htmlspecialchars($media_item['file_path']); ?>"
            ...
            <?php if (!empty($related_plants)): ?>
                <div class="related-species-section"
                    style="margin-top: 40px; padding-top: 20px; border-top: 2px solid #eee;">
                    ...
                    <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                        <?php foreach ($related_plants as $rel_plant): ?>
                            ...
                            <a href="plant_details.php?id=<?php echo $rel_plant['id']; ?>"
                                style="text-decoration: none; color: inherit; background: #f9f9f9; border-radius: 8px; padding: 10px; width: 120px; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1); transition: transform 0.2s; display: block;">
                                <img src="<?php echo htmlspecialchars($img_path); ?>"
                                    alt="<?php echo htmlspecialchars($rel_plant['common_name']); ?>"
                                    style="width: 100%; height: 80px; object-fit: cover; border-radius: 5px; margin-bottom: 8px;">
                                <strong
                                    style="font-size: 0.9em; display: block; color: #333;"><?php echo htmlspecialchars($rel_plant['common_name']); ?></strong>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
